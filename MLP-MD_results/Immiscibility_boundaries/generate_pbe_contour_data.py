...
PHASE_SEP_DATA_PBE_FULL = np.array([
    # [Pressure, Temperature, First zero crossing from left, First zero crossing from right]
    	
    [100, 1000, 0.070, 0.798],
    [100, 2000, 0.070, 0.572],
    [100, 3000, 0.102, 0.557],
    [100, 4000, 0.136, 0.559],
    [100, 5000, 0.315, 0.533],
    [150, 2000, 0.045, 0.744],
    [150, 3000, 0.070, 0.746],
    [150, 4000, 0.106, 0.736],
    [150, 5000, 0.171, 0.583],
    [200, 2000, 0.062, 0.805],
    [200, 3000, 0.068, 0.802],
    [200, 4000, 0.078, 0.802],
    [200, 5000, 0.139, 0.750],
    [200, 6000, 0.287, 0.621],
    [400, 2000, 0.037, 0.789],
    [400, 3000, 0.069, 0.830],
    [400, 4000, 0.072, 0.812],
    [400, 5000, 0.117, 0.805],
    [400, 6000, 0.153, 0.768],
    [400, 7000, 0.179, 0.588],
    [600, 2000, 0.038, 0.748],
    [600, 3000, 0.067, 0.871],
    [600, 4000, 0.072, 0.863],
    [600, 5000, 0.100, 0.820],
    [600, 6000, 0.151, 0.804],
    [600, 7000, 0.181, 0.757],
    [600, 8000, 0.342, 0.596],
    [800, 2000, 0.033, 0.799],
    [800, 3000, 0.072, 0.800],
    [800, 4000, 0.071, 0.873],
    [800, 5000, 0.076, 0.866],
    [800, 6000, 0.150, 0.816],
    [800, 7000, 0.196, 0.804],
    [800, 8000, 0.256, 0.668],
    [1000, 2000, 0.042, 0.800],
    [1000, 3000, 0.063, 0.862],
    [1000, 4000, 0.070, 0.924],
    [1000, 5000, 0.106, 0.866],
    [1000, 6000, 0.137, 0.822],
    [1000, 7000, 0.207, 0.753],
    [1000, 8000, 0.187, 0.800],
])


UPPER_BOUND_PBE = (
    np.array([100.0, 150.0, 200.0, 400.0, 600.0, 800.0, 1000.0], dtype=float),
    np.array([4000.0, 5000.0, 6000.0, 7000.0, 8000.0, 8000.0, 8000.0], dtype=float),
)


def get_upper_bound_temperature(pressure: float) -> float:
    p_ref, t_upper_ref = UPPER_BOUND_PBE
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate PBE_contour_data.csv from hardcoded PBE phase-separation data"
    )
    parser.add_argument(
        "--output",
        default="PBE_contour_data.csv",
        help="Output CSV path (default: PBE_contour_data.csv)",
    )
    parser.add_argument(
        "--x-targets",
        nargs="+",
...
def main() -> None:
    ...
    for x_target in fitted_targets:
        t_fit = fit_temperature_at_p150(PHASE_SEP_DATA_PBE_FULL, x_target, t_min=2000.0)
        if t_fit is None:
            print(
